...
protected:
  ...
  using _Base::_M_allocate;//SGI STL的空间配置器
...
public:
  ...
  void push_back(const _Tp& __x) {
    if (_M_finish != _M_end_of_storage) {//如果有备用空间
...
vector<_Tp, _Alloc>::_M_insert_aux(iterator __position, const _Tp& __x)
{
  if (_M_finish != _M_end_of_storage) {//还有备用空间
    construct(_M_finish, *(_M_finish - 1));//在备用空间起始处构造一个元素，并以vector最后一个元素值为其初值
    ++_M_finish;
    _Tp __x_copy = __x;
  ...
  /*
	...
	*/
    ...
    __STL_TRY {
      ...
      construct(__new_finish, __x);//为新元素设定初值__x
      ++__new_finish;
      __new_finish = uninitialized_copy(__position, _M_finish, __new_finish);//将原vector的备用空间中的内容也拷贝过来（？？？）
    }
    __STL_UNWIND((destroy(__new_start,__new_finish), 
                  _M_deallocate(__new_start,__len)));
    destroy(begin(), end());
    _M_deallocate(_M_start, _M_end_of_storage - _M_start);//释放并析构原vector
  ...
  if (__n != 0) {//当n!=0才进行以下所有操作
    if (size_type(_M_end_of_storage - _M_finish) >= __n) {//备用空间大于等于新增元素个数
      ...
      if (__elems_after > __n) {//插入点之后的现有元素个数大于新增元素个数
        ...
        fill(__position, __position + __n, __x_copy);//从插入点开始填入新值
      }
