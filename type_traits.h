...
struct __type_traits { 
   ...
   /* The following restrictions should be observed for the sake of
	...
	/*
		不要移除这个成员，它通知“有能力自动将__type_traits特化”的编译器说，我们现在所看到的这个__type_traits template
		是特殊的，这是为了确保万一编译器也使用一个名为__type_traits而其实于此处定义并无任何关联的template时，所有的事情
		都仍将顺利运作。
	*/
	/*
				   以下条件应被遵守，因为编译器有可能自动为各型别产生专属的__type_traits特化版本：
					1.你可以重新排列以下的成员次序；
					2.你可以移除以下任何成员；
					3.绝对不可以将以下成员重新命名而却没有改变编译器中的对应名称；
					4.新加入的成员会被视为一般成员，除非你在编译器中加上适当地支持。
				   */
...
//  that don't.
/*
以下针对C++基本型别char,signed char,unsigned char,short,unsigned short,int ,unsigned int,long,unsigned long,
float,double,long double提供特化版本，注意每一个成员的值都是__ture_type，表示这些型别都可采用最快速方式（如
memcpy）来进行拷贝或赋值操作。
*/
