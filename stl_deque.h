...
struct _Deque_iterator {
  ...
  _Self& operator++() {
    ++_M_cur;//切换至下一个元素
    if (_M_cur == _M_last) {//如果已达所在缓冲区的尾链
      _M_set_node(_M_node + 1);//切换至下一节点（即缓冲区）
...
protected:
  ...
  using _Base::_M_map;//指向map，map是块连续空间，其每个元素都是指针，指向一个节点（缓冲区）
  using _Base::_M_map_size;//map内有多少指针
