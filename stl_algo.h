...
/*
rotate可能是STL的所有算法中最具有自适应性的算法：
	对前向迭代器来说，它执行n此交换以旋转n个元素；
...
// random_shuffle
//使用默认的随机数生成器来打乱[first,last)之间的元素顺序，默认随机数生成器和具体的编译器实现有关
template <class _RandomAccessIter>
...
//使用指定的随机数生成函数__rand来打乱[first,last)之间的元素的顺序，__rand接受一个difference_type类型的参数n,返回一个随机数x,x的范围在[0,n)之间
...
//将范围内的元素分为两个不相交的集，满足一元谓词的元素在不满足一元谓词的元素之前。
template <class _ForwardIter, class _Predicate>
...
//stable_partition是少数几个在头文件<algorithm>中定义的，在可能的情况下能够利用临时缓冲区获益的模板函数中的一个。
template <class _ForwardIter, class _Predicate>
...
/*
nth_element执行一种通常的“分而治之”的排序方法。在这种情况下，算法将忽略调所有不包括
第n个元素的子区间。一旦它创建了一个能够容纳该元素的足够小的划分，就会调用insertion_sort来完成（部分排序）工作。
*/
template <class _RandomAccessIter>
